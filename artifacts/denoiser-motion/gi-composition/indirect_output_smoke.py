...
def main():
    ...
    try:
        for mode in ("radiance", "history", "validity", "acceptance", "off"):
            results = []
            for custom in (False, True):
                ...
                config = replace(config, wavefront_indirect_reuse_storage=True, wavefront_indirect_reuse_candidates=True, wavefront_indirect_reuse_debug_view=mode, wavefront_indirect_reuse_apply=(mode=="off"))
                calls = []
                with ol.VulkanGlfwPresenter(window, config=config) as presenter:
                    ...
                    assert presenter._core.wavefront_executor is None or presenter._core.wavefront_executor.indirect_reuse_clear_pipeline is None
                    frames = []
