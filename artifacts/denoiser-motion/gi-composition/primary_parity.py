...
if os.environ.get("PRIMARY_PARITY_CONTROL") == "direct":
    composed_primary = direct_primary
elif os.environ.get("PRIMARY_PARITY_CONTROL") == "composed":
    direct_primary = composed_primary
...
def main():
    ...
    try:
        for mode in ("bilinear", "fsr1-shade", "fsr2"):
            ...
            delta = np.abs(results[0].astype(float) - results[1].astype(float))
            print(
                mode,
                "difference",
                float(delta.max()),
        ...
        primary_graph.record_primary = composed_primary
