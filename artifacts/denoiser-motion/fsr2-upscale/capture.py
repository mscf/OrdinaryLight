...
out = Path(os.environ.get("OUT", "/tmp/fsr2-upscale"))
...
report = {}
sheet = Image.new("RGB", (1920, 1530))
try:
    for row, subject in enumerate(("glass", "target", "camera")):
        captures = {}
        for col, mode in enumerate(("bilinear", "fsr1", "fsr2")):
            ...
            with ol.VulkanGlfwPresenter(w, config=cfg) as p:
                for x in list(np.linspace(-0.3, 0.3, 12)) + [0.3] * 12:
                    p.present_wavefront(
                        scene, pose(scene, glass, bars, subject, float(x)), 640, 480
                    )
            ...
            if col == 0:
                ...
                assert np.array_equal(baseline, hdr), (
                    "spatial upscale changed internal HDR"
                )
            sheet.paste(
                Image.fromarray(frames[-1][..., :3]), (640 * col, 510 * row + 30)
            )
            ImageDraw.Draw(sheet).text(
                (640 * col + 8, 510 * row + 8), f"{subject}: {mode}", fill="white"
            )
        a = captures["bilinear"]
        b = captures["fsr2"]
        assert np.any(a != b), "FSR 2 did not change output"
        report[subject] = {
            "mean_absolute_display_difference": float(
                np.abs(a.astype(float) - b).mean()
            ),
            "fsr2_uses_frame_jitter": True,
        }
