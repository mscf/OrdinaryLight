...
w = g.create_window(3840, 2160, "Upscale profile", None, None)
result = []
try:
    for mode in ("fsr1", "fsr2", "fsr2", "fsr1"):
        ...
        with ol.VulkanGlfwPresenter(w, config=config) as p:
            for i in range(24):
                p.present_wavefront(
                    scene,
                    pose(scene, glass, bars, "camera", -0.3 + 0.6 * (i % 12) / 11),
                    3840,
                    2160,
                )
                if i >= 8:
                    ...
                    samples.append(
                        {
                            "gpu_ms": t["gpu_frame_ms"],
                            "reconstruct_ms": t["wavefront_stage_ms"]["reconstruct"]
                            + t["wavefront_stage_ms"].get("fsr2", 0),
                        }
                    )
        result.append(
            {
                "filter": mode,
                "samples": samples,
                "median_reconstruct_ms": float(
                    np.median([s["reconstruct_ms"] for s in samples])
                ),
                "median_gpu_ms": float(np.median([s["gpu_ms"] for s in samples])),
            }
        )
        print(result[-1], flush=True)
    Path("/tmp/fsr2-profile.json").write_text(json.dumps(result, indent=2))
finally:
    g.destroy_window(w)
    g.terminate()
