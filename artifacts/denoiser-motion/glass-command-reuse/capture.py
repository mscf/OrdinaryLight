...
try:
 ...
 with ol.VulkanGlfwPresenter(w,config=cfg) as p:
  ...
  for phase in ['moving','stationary']:
   ...
   for i in range(int(os.environ.get('FRAMES','32'))):
    ...
    stage.append({k:v for k,v in p._core.last_timings.items() if k.startswith('scene_partial') or k == 'wavefront_command_cache_hit'})
