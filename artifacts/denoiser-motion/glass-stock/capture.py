...
out=Path('/tmp/glass-stock');out.mkdir(exist_ok=True)
...
try:
 for mode in ['off','on']:
  ...
  for mesh in scene.meshes:
   scene.update_instance(mesh,material=replace(mesh.material,program=None))
  if mode=='reordered':
  ...
  with ol.VulkanGlfwPresenter(w,config=replace(cfg,denoiser_transmission_motion_cap=mode!='off')) as p:
   ...
   assert p._core.wavefront_executor.custom_primary_pipeline is None
