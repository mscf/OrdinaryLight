...
out=Path('/tmp/glass-adaptive-640');out.mkdir(exist_ok=True)
...
cfg=replace(_gi_config(SimpleNamespace(renderer={},id='glass-detail'),capture=True),denoiser_transmission_motion_cap=True,direct_swapchain_storage=False,wavefront_tile_capacity=640*480)
...
try:
 for row,kind in enumerate(['camera','glass','target']):
  scene,glass,bars=fixture();cam=pose(scene,glass,bars,kind,.3);refs=[]
  ...
  with ol.VulkanGlfwPresenter(window,config=refcfg) as p:
   for i in range(8):
    p._core.wavefront_frame_sequence=2000+i;p.present_wavefront(scene,cam,640,480);refs.append(p.capture_wavefront_hdr()[...,:3])
  refs=np.asarray([refs,refs]);np.save(out/(kind+'-references.npy'),refs)
  metrics[kind]={}
