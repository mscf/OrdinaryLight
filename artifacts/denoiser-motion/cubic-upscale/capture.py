...
out=Path(os.environ.get('OUT', '/tmp/cubic-upscale'));out.mkdir(exist_ok=True)
...
report={};sheet=Image.new('RGB',(1280,1530))
try:
 for row,subject in enumerate(('glass','target','camera')):
  captures={}
  for col,mode in enumerate(('bilinear','clamped-cubic')):
