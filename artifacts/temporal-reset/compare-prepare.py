...
import prepare_variants as variants
...
with ExitStack() as stack:
 ...
 for n,r in rs.items():
  variants.current=128 if n else 64
 ...
 for n,data in rows.items():
  print(json.dumps(dict(prepare_workgroup=128 if n else 64,samples=len(data),gpu={k:dict(median=float(np.median([x[k] for x in data])),p95=float(np.percentile([x[k] for x in data],95))) for k in ('total','native_gi','primary','intersect_shade.1','gpu_population','face_output','resolve_hdr','relax_prepare')})),flush=True)
