...
def prepare_material_resources(
    ...
    from .layout import MaterialResourceLayout

    layout = MaterialResourceLayout.from_programs(
        programs, descriptor_set=descriptor_set, first_binding=first_binding
    )
    declarations = {
        declaration.name: declaration for declaration in layout.declarations
    }
    supplied = dict(supplied or {})
    if supplied.keys() != declarations.keys():
        raise ValueError("Material resources must exactly match graph declarations")
    bindings, owners = {}, []
    for declaration, binding in layout.entries:
    ...
    for owner in owners:
        ...
        owner.require_open()
    return bindings, layout.source, tuple(dict.fromkeys(owners))
...
class VulkanMaterialResources:
    ...
    def __init__(self, runtime, programs, resources):
        ...
        self.resource_layout = MaterialResourceLayout.from_programs(self.programs)
        self.declarations = self.resource_layout.declarations
        self._borrowers = set()
    ...
    def validate(self, programs):
        self.require_open()
        self.resource_layout.validate(programs)
    ...
    @property
    def uses(self):
        """Read dependencies for graph consumers, including sampled layouts."""
        from ..transport._custom_resources import resource_uses

        self.require_open()
        return resource_uses(self._descriptors.bindings)

    def bind_graph(self, command, pipeline_layout):
        """Bind set 1 after the graph has recorded the dependencies in uses.

        Layout publication happens after submission, so graph recording must not
        consult the allocations' still-uncommitted layout fields here.
        """
        self.require_open()
