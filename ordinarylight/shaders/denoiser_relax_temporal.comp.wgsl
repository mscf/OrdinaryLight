...
fn main(
    ...
    let motion_vector: vec2<f32> = textureLoad(motion, pixel).xy;
    ...
    var accepted: bool = (((constants.extent_history.w > 0.5) && in_bounds) && (current_depth != 0.0));
    ...
    if (accepted) {
        ...
        let depth_tolerance: f32 = max((abs(current_depth) * constants.rejection.y), 0.001);
        accepted = ((((old_depth != 0.0) && (dot(current_normal, old_normal) >= constants.rejection.x)) && (abs((current_depth - old_depth)) <= depth_tolerance)) && (old_material == current_material));
        if (accepted) {
