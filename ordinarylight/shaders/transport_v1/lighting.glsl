...
{
    if (OL_TRANSPORT_AREA_LIGHT_COUNT == 0u) return vec3(0.0);
    float selection = (float(sample_index) + randomFloat(rng)) /
        float(sample_count);
    uint lower = 0u, upper = OL_TRANSPORT_AREA_LIGHT_COUNT - 1u;
...
float unifiedAreaDomainProbability()
{
    ...
    return ordinarylight_unified_area_probability(
        OL_TRANSPORT_AREA_LIGHT_COUNT, OL_TRANSPORT_ENVIRONMENT_SAMPLES,
        OL_TRANSPORT_AREA_LIGHT_WEIGHT);
#else
    if (OL_TRANSPORT_AREA_LIGHT_COUNT == 0u)
        return 0.0;
    if (OL_TRANSPORT_ENVIRONMENT_SAMPLES == 0u)
        return 1.0;
    float area_weight = sqrt(max(OL_TRANSPORT_AREA_LIGHT_WEIGHT, 0.000001));
...
vec3 sampleUnifiedSecondaryLight(
    ...
    inout uint rng)
{
    bool area_enabled = OL_TRANSPORT_AREA_LIGHT_COUNT > 0u;
    bool environment_enabled = OL_TRANSPORT_ENVIRONMENT_SAMPLES > 0u;
    ...
    float area_probability = ordinarylight_unified_secondary_area_probability(
        area_enabled, environment_enabled, OL_TRANSPORT_AREA_LIGHT_WEIGHT,
        OL_TRANSPORT_SECONDARY_AREA_LIGHT_SAMPLES, OL_TRANSPORT_ENVIRONMENT_SAMPLES);
#else
    float area_weight = area_enabled
        ? sqrt(max(OL_TRANSPORT_AREA_LIGHT_WEIGHT, 0.000001))
            * float(max(OL_TRANSPORT_SECONDARY_AREA_LIGHT_SAMPLES, 1u)) : 0.0;
    float environment_weight = environment_enabled
        ? float(min(OL_TRANSPORT_ENVIRONMENT_SAMPLES, 4u)) : 0.0;
