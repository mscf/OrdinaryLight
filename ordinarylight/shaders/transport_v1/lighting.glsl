...
#endif
        bool shadow = nativeOccluded(shadow_origin, 0.001, direction, shadow_distance, nativeSurfaceMask());
        if (shadow)
        {
            index = (index + 1);
            continue;
...
vec3 sampleAreaLightTechnique(vec3 hit, vec3 normal, vec3 incoming, MaterialData material, inout uint rng, uint sample_index, uint sample_count, float technique_probability)
{
    ...
    if ((nativeAreaLightCount(OL_TRANSPORT_AREA_LIGHT_COUNT) == uint(0)))
    {
        return vec3(0.0);
    }
    float custom_selector = ((float(sample_index) + randomFloat(rng)) / float(sample_count));
    AreaLightCandidate custom_candidate = AreaLightCandidate(nativeSelectEmitter(custom_selector), vec2(randomFloat(rng), randomFloat(rng)), 0.0);
    vec3 custom_direction = normal;
    float custom_distance_to_light = 0.0;
    vec3 custom_contribution = evaluateAreaLightCandidateTechnique(custom_candidate, hit, normal, incoming, material, sample_count, technique_probability, custom_direction, custom_distance_to_light);
    if (all(equal(custom_contribution, vec3(0.0))))
    {
        return vec3(0.0);
    }
    ...
    uint lower = uint(0);
    uint upper = (nativeAreaLightCount(OL_TRANSPORT_AREA_LIGHT_COUNT) - uint(1));
    uint step = uint(0);
...
float unifiedAreaDomainProbability()
{
    ...
    return ((OL_TRANSPORT_ENVIRONMENT_SAMPLES == uint(0)) ? 1.0 : 0.5);
#endif
...
vec3 evaluateAreaLightCandidateTechnique(AreaLightCandidate candidate, vec3 hit, vec3 normal, vec3 incoming, MaterialData material, uint sample_count, float technique_probability, out vec3 direction, out float distance_to_light)
{
...
#if WAVE_NATIVE_EMITTERS
    NativeEmitterSample custom_sample = nativeEvaluateEmitter(candidate.light_index, candidate.barycentrics);
    direction = normal;
    distance_to_light = 0.0;
    ...
    if ((!nativeEmitterInfluence(candidate.light_index, custom_sample.position, hit)))
    {
        return vec3(0.0);
    }
    vec3 custom_offset = (custom_sample.position - hit);
    float custom_distance_squared = dot(custom_offset, custom_offset);
    distance_to_light = sqrt(custom_distance_squared);
    direction = (custom_offset / max(distance_to_light, 1e-06));
    float custom_surface_cosine = max(dot(normal, direction), 0.0);
    float custom_light_cosine = dot(custom_sample.normal, (-direction));
    custom_light_cosine = (custom_sample.two_sided ? abs(custom_light_cosine) : max(custom_light_cosine, 0.0));
    if (((custom_surface_cosine <= 0.0) || (custom_light_cosine <= 1e-06)))
    {
        return vec3(0.0);
    }
    float custom_effective_pdf = (((custom_sample.area_pdf * custom_distance_squared) / custom_light_cosine) * technique_probability);
    float custom_bsdf_pdf = pbrPdf(material, normal, (-incoming), direction);
    float custom_mis = ordinarylight_area_light_mis(custom_effective_pdf, 1.0, custom_bsdf_pdf);
    return ordinarylight_area_light_contribution(evaluatePbr(material, normal, (-incoming), direction), custom_sample.emission, custom_surface_cosine, custom_mis, 1.0, custom_effective_pdf);
#endif
...
AreaLightCandidate generateAreaLightCandidate(vec3 hit, vec3 normal, vec3 incoming, MaterialData material, inout uint rng, uint sample_index, uint sample_count)
{
    ...
    candidate.barycentrics = unpackHalf2x16(packHalf2x16(vec2(randomFloat(rng), randomFloat(rng))));
    vec3 custom_direction = normal;
    float custom_distance = 0.0;
    vec3 custom_contribution = evaluateAreaLightCandidate(candidate, hit, normal, incoming, material, sample_count, custom_direction, custom_distance);
...
vec3 sampleEnvironmentTechnique(vec3 hit, vec3 normal, vec3 incoming, MaterialData material, inout uint rng, uint sample_count, float technique_probability)
{
    ...
    vec3 environment_radiance = environmentColor(direction);
    if (all(equal(environment_radiance, vec3(0.0))))
    {
        return vec3(0.0);
    }
#endif
    ...
    profileWork(uint(1), uint(1));
#endif
    bool shadow = nativeOccluded(shadow_origin, 0.001, direction, 1e+30, nativeSurfaceMask());
    if (shadow)
    {
        return vec3(0.0);
    }
...
{
    bool area_enabled = (nativeAreaLightCount(OL_TRANSPORT_AREA_LIGHT_COUNT) > uint(0));
