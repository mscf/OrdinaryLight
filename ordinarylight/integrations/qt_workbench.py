...
def _initial_showcase_index(entries, requested=None):
    """Resolve an optional showcase id, retaining the visual baseline default."""
    requested = str(requested or "").strip()
    preferred = requested or "area-lights"
    return next(
        (
            index for index, entry in enumerate(entries)
            if entry.showcase is not None and entry.showcase.id == preferred
        ),
        0,
    )
    ...
    class Workbench(QtWidgets.QMainWindow):
        ...
        def _populate_builtins(self, *, select=True):
            ...
            if select and self.state.scenes:
                ...
                baseline_row = _initial_showcase_index(
                    self.state.scenes,
                    os.environ.get("ORDINARYLIGHT_WORKBENCH_SHOWCASE"),
                )
