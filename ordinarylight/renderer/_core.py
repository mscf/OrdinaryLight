"""High-level, renderer-neutral rendering interface."""
...
from ..capabilities import capabilities_from_renderer
from ..renderers.base import RendererProtocol
...
def _default_implementation(config, config_options, preference="auto"):
    """Construct the preferred Vulkan renderer without eager Vulkan imports."""
    try:
        from ..renderers.selection import select_vulkan_renderer
        from ..targets.vulkan import RendererConfig
    except ImportError as error:
        if error.name == "vulkan":
            raise RuntimeError(
                "The default renderer requires the Vulkan extra; install "
                "ordinarylight[vulkan], or pass an explicit implementation"
            ) from error
        raise
    resolved_config = config or RendererConfig(**config_options)
    return select_vulkan_renderer(
        preference, config=resolved_config,
    )


class RenderStatistics(Mapping):
    """Immutable, renderer-neutral measurements for one completed render."""
    ...
    @property
    def timings(self):
        """Complete implementation-specific timing measurements."""
...
class RenderJob:
    ...
    Calls submitted through one renderer execute in order so an implementation never
...
class Renderer:
    """Render scenes to HDR NumPy arrays without exposing implementation mechanics.

    By default this class owns the optional Vulkan implementation. A compatible
    :class:`ordinarylight.renderers.RendererProtocol` may be supplied explicitly.
    ...
    def __init__(
        ...
        *,
        ...
        implementation: RendererProtocol | None = None,
        ...
        **config_options,
        ...
        if implementation is not None and (config is not None or config_options):
            raise TypeError("a supplied implementation owns its configuration")
        if implementation is not None and renderer_preference != "auto":
            raise TypeError(
                "renderer_preference cannot be used with a supplied implementation"
            )
        self._implementation = (
            implementation if implementation is not None
            else _default_implementation(config, config_options, renderer_preference)
        )
        if not callable(getattr(self._implementation, "render_frame", None)):
            raise TypeError("implementation must implement RendererProtocol.render_frame()")
        if not callable(getattr(self._implementation, "close", None)):
            raise TypeError("implementation must provide close()")
        self._frame_index = 0
        self._last_statistics = None
        self._capabilities = capabilities_from_renderer(self._implementation)
        self._state_lock = Lock()
        self._submission_lock = Lock()
        self._implementation_lock = Lock()
    ...
    @property
    def config(self):
        """The immutable configuration owned by the active renderer implementation."""
        return getattr(self._implementation, "config", None)

    @property
    def device(self):
        """Selected device information, when exposed by the renderer implementation."""
    ...
    def available_outputs(self):
        """Named products supported by the active renderer implementation."""
    ...
    def renderer_selection(self):
        """Explain an automatic renderer choice and any compatibility fallback."""
        return self._capabilities.selection

    @property
    def accumulation_state(self):
        """Current stationary-accumulation state reported by the renderer implementation."""
        return getattr(
            self._implementation, "accumulation_state", AccumulationState.DISABLED
        )

    @property
    def accumulated_frames(self):
        """Frames represented by the current progressive history."""
        return int(getattr(self._implementation, "accumulated_frames", 0))

    @property
    def effective_samples_per_pixel(self):
        """Sample budget selected for the most recently completed frame."""
        return int(getattr(
            self._implementation, "effective_samples_per_pixel",
    ...
    def reset_sequence(self, frame_index=0):
        ...
        with self._submission_lock:
            ...
            self._executor.submit(self._reset_implementation_history).result()

    def replace_scene(self, scene: Scene, *, reset_sequence=True):
        """Make ``scene`` resident without recreating this renderer.

        Vulkan renderers retain their device, compiled pipelines, presentation
    ...
    def _replace_scene_serialized(self, scene):
        with self._implementation_lock:
            replace_scene = getattr(self._implementation, "replace_scene", None)
            if not callable(replace_scene):
                raise RuntimeError(
                    "the active renderer implementation does not support "
                    "resident scene replacement"
                )
            replace_scene(scene)
            self._last_statistics = None

    def reconfigure(self, **changes):
        """Apply settings supported by the resident renderer in place.
    ...
    def _reconfigure_serialized(self, changes):
        with self._implementation_lock:
            reconfigure = getattr(self._implementation, "reconfigure", None)
            if not callable(reconfigure):
                raise RuntimeError(
                    "the active renderer implementation requires renderer recreation for settings"
                )
    ...
    def pick_async(
        ...
        Accelerated renderers use their resident scene acceleration structure;
        other renderers transparently use the portable CPU picker.
        """
        if not isinstance(scene, Scene):
            raise TypeError("scene must be a Scene")
    ...
    def _pick_serialized(
        ...
        with self._implementation_lock:
            operation = getattr(self._implementation, "pick", None)
            result = (
                operation(
    ...
    def _object_effects_serialized(self, scene, bindings):
        with self._implementation_lock:
            if not bindings:
                operation = getattr(self._implementation, "clear_object_effect", None)
                if callable(operation):
                    return operation()
            operation = getattr(self._implementation, "set_object_effects", None)
            if callable(operation):
                return operation(scene, bindings)
            if len(bindings) == 1:
                operation = getattr(self._implementation, "apply_object_effect", None)
                if callable(operation):
    ...
    def render_to(self, scene, camera, surface, *, samples=None, frame_index=None):
        """Render and present to a renderer-neutral :class:`RenderSurface`."""
    ...
    def render_gpu(
        ...
        This optional renderer capability returns a
        ...
        with self._submission_lock:
            with self._state_lock:
                ...
                if current_frame < 0:
                    raise ValueError("frame_index cannot be negative")
                self._frame_index = current_frame + 1
            render_gpu_frame = getattr(self._implementation, "render_gpu_frame", None)
            if not callable(render_gpu_frame):
                raise RuntimeError(
                    "the active renderer implementation does not support GPU-resident output"
                )
            with self._implementation_lock:
    ...
    def _render_now(
        ...
        if named and callable(getattr(self._implementation, "render_products", None)):
            products = self._implementation.render_products(
                scene, camera, width, height, outputs=requested,
                samples=samples, frame_index=current_frame,
            )
        else:
            products = {"color": self._implementation.render_frame(
        ...
        for name, result in products.items():
            ...
            if result.shape != shape or result.dtype != dtype:
                raise RuntimeError(
                    f"renderer returned invalid {name!r}; expected {dtype} "
