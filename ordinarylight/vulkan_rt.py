...
class VulkanWavefrontExecutor:
    ...
    def __init__(self, core, capacity):
        ...
        self.secondary_path_buffer = core._create_buffer(
            (capacity if core.config.wavefront_indirect_reuse_candidates else 1)
    ...
    def ensure_custom_material_pipelines(self):
        ...
        if layout is not None:
            from .shader_compiler import compile_wavefront_material_shader
    ...
    def _create_pipelines(self):
        ...
        if not native_suffix and not profile_suffix:
            (
        ...
        if self.core.config.wavefront_megakernel_single_warp:
            (
        ...
        if self.indirect_reuse_clear_layout:
            self.indirect_reuse_clear_sets = list(
                allocated[shade_end + 8:shade_end + 10]
            )
        if self.indirect_reuse_candidate_layout:
            candidate_start = shade_end + 8 + (
                2 if self.indirect_reuse_clear_layout else 0
            )
            self.indirect_reuse_candidate_sets = list(
                allocated[candidate_start:candidate_start + 2]
            )
        if self.indirect_reuse_debug_layout:
            debug_start = shade_end + 8
            if self.indirect_reuse_clear_layout:
                debug_start += 2
            if self.indirect_reuse_candidate_layout:
                debug_start += 2
            self.indirect_reuse_debug_sets = list(
                allocated[debug_start:debug_start + 2]
            )
        if self.bucket_intersect_layout:
            coherent_layouts = (
                [self.shade_layout] * (
    ...
    def record_reconstruction(
        self, command, slot, source_width, source_height,
        output_width, output_height, history_valid=False,
    ):
    ...
    def dispatch(
        ...
        if primary_hit_readback:
            return result + (self._read_primary_hits(primary_count),)
        return result
...
class VulkanRayQueryCore:
    ...
    def resolved_execution_strategy(self):
        from .vulkan import _resolve_execution_strategy
    ...
    def _replace_compute_pipeline(self, programs, *, attribute_layout=None):
        ...
        if len(programs) == 1 and programs[0] is builtin_material:
            ...
            from .shader_compiler import compile_material_shader
        ...
        if self.shader_module:
            ...
            self.shader_module = None
        self.shader_module = vk.vkCreateShaderModule(
            self.device,
            vk.VkShaderModuleCreateInfo(
                sType=vk.VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO,
    ...
    def trace_wavefront_tile(
        ...
        constants = bytearray(struct.pack(
            ...
            tile_width, tile_height, int(frame_index), int(sample_index),
        ))
    ...
    def upload_window_scene(self, scene):
        """Replace the persistent GPU scene, synchronizing before destruction.

        The historical name is retained for compatibility, but persistent scene
        resources are also used by offscreen wavefront validation renders.
        """
        if (
        ...
        self.scene_resources = VulkanSceneResources(self, scene)
        self.scene_tlas = self.scene_resources.tlas
        self.scene_vertex_buffer = self.scene_resources.vertex_buffer
        self.scene_material_buffer = self.scene_resources.material_buffer
        self.scene_light_buffer = self.scene_resources.light_buffer
        self.scene_area_light_buffer = self.scene_resources.area_light_buffer
        self.scene_attribute_buffer = self.scene_resources.attribute_buffer
        self.scene_custom_attribute_buffer = (
            self.scene_resources.custom_attribute_buffer
        )
        self.scene_custom_attribute_layout = (
            self.scene_resources.custom_attribute_layout
        )
        self.scene_texture_buffer = self.scene_resources.texture_buffer
        self.scene_texture_binding_buffer = self.scene_resources.texture_binding_buffer
        self.scene_volume_header_buffer = self.scene_resources.volume_header_buffer
        self.scene_volume_scalar_buffer = self.scene_resources.volume_scalar_buffer
        self.scene_volume_transfer_buffer = self.scene_resources.volume_transfer_buffer
        self.scene_triangle_volume_buffer = self.scene_resources.triangle_volume_buffer
        self.scene_volume_empty_space_skipping = (
            self.scene_resources.volume_empty_space_skipping
        )
    ...
    def create_window_swapchain(
        ...
        for frame in self.window_frames:
            ...
            reservoir_bytes = (
                render_width * render_height * (
                    12 + (8 if self.config.wavefront_stratified_primary_restir else 0)
        ...
        self._single_use(
            lambda command: vk.vkCmdPipelineBarrier(
                ...
                vk.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                0, 0, None, 0, None,
                len(self.window_frames) * 6 + len(self.accumulation_images)
