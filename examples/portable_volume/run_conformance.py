...
def main():
    ...
    parser.add_argument(
        "--headless",
        action="store_true",
        help="Also run hardware Chrome without a window",
    )
    ...
    with ExitStack() as stack:
        profile = Path(
            stack.enter_context(tempfile.TemporaryDirectory(prefix="portable-chrome-"))
        )
        ...
        cdp_port = wait_for(
            browser,
            lambda: (profile / "DevToolsActivePort").read_text().splitlines()[0],
        )
        commands = [
            [
                args.node,
                "--experimental-websocket",
                str(example / "browser_probe.mjs"),
                f"http://127.0.0.1:{cdp_port}",
                url,
                str(root / "browser.json"),
            ],
        ...
        report["chrome_version"] = subprocess.check_output(
            [args.chrome, "--version"], text=True
        ).strip()
