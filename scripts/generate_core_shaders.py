...
"""
    ...
    if shader is wavefront_shade_candidate:
        start = source.index("struct MaterialEvaluation\n")
        end = source.index("\n};", start) + 3
        declaration = source[start:end]
        contract = (ROOT / "ordinarylight/shaders/transport_v1/material_contracts.glsl").read_text()
        if declaration not in contract:
            raise ValueError("Update the shared material contract to match the typed MaterialEvaluation ABI")
    marker = (
