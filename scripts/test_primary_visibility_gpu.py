"""Headless triangle transport check for the split-primary integration harness."""
...
@pytest.mark.parametrize("replay_shape",((8,8),(16,4),(32,2)))
@pytest.mark.parametrize("capture_mode", ("adjacent","tiled","single"))
def test_triangle_samples_history_and_resize(capture_mode,replay_shape,cache_format):
    import vulkan as vk
    ...
    with ExitStack() as stack:
        install(stack, lambda: enabled, full_frame=capture_mode!="adjacent",replay_workgroup=lambda:replay_shape,single_capture=lambda:capture_mode=="single",visibility_format=lambda:cache_format)
        ...
        for mode in ('fused', 'split'):
            enabled = mode == 'split'
            pipeline = stack.enter_context(VulkanWavefrontPipeline(runtime, resident, config=config))
            rows[mode] = []
